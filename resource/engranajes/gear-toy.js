...
(function ($) {
  ...
  function main() {
    ...
    _randomiseInterval = setInterval(function () {
      if (_autoShuffle) _randomiseScene(true);
    }, 4000);

    // start the d3 animation timer
    d3.timer(function () {
      _svg.selectAll(".gear-path").attr("transform", function (d) {
        d.angle += d.speed;
  ...
  var _generateScene = function (options) {
    var holeRadius, teeth, radius, factor, newGear, innerRadius;
  ...
  var _randomiseScene = function (transition) {
    ...
    _svg.selectAll(".gear").each(function (d, i) {
      if (transition) {
        d3.select(this)
          .transition()
          .ease("elastic")
  ...
  var _initGui = function () {
    if (!window.dat) {
      console.log(
        "Could not create GUI. Check dat.gui library is loaded first."
      );
      return;
    }
    ...
    var funcs = {
      ...
      updateSpeed: function (speed) {
        $.each(_allGears, function () {
          ...
          if (datum.power !== 0) datum.power = speed;
        });
    ...
    controls
      .add(_options, "number", 1, 50)
      .onFinishChange(funcs.onFinishChange);
    controls
      .add(_options, "holeRadius", 0, 0.9)
      .onFinishChange(funcs.onFinishChange);
    controls
      .add(_options, "addendum", 0, 16)
      .onFinishChange(funcs.onFinishChange);
    controls
      .add(_options, "dedendum", 0, 16)
      .onFinishChange(funcs.onFinishChange);
    controls
      .add(_options, "thickness", 0, 0.8)
      .onFinishChange(funcs.onFinishChange);
    controls
      .add(_options, "profileSlope", 0, 0.8)
      .onFinishChange(funcs.onFinishChange);

    controls
      .add(_options, "speed", 0.001, 0.2)
      .step(0.001)
